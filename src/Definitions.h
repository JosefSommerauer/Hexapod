...
int const NUM_OF_SERVO_VALS = 5; //4;
...
struct TAngles{
	...
	TAngles operator - (const TAngles& rhs)	{
		return TAngles(
			this->AngleFemur - rhs.AngleFemur,
			this->AngleTibia - rhs.AngleTibia,
			this->AngleTarsus - rhs.AngleTarsus
		);
	}
	...
	TAngles operator / (const int rhs)	{
		return TAngles(
			this->AngleFemur / rhs,
			this->AngleTibia / rhs,
			this->AngleTarsus / rhs
		);
	}
