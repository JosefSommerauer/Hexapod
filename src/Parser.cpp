    ...
    if(file.is_open()) {
        ...
        for(int i = 0; i < NUM_OF_LEGS*3; timeout++) {
            ...
            if(sscanf(line, ";%d, %d, %d, %d, %d;",
            		&mServoValues[i][0], &mServoValues[i][1], &mServoValues[i][2], 
						&mServoValues[i][3], &mServoValues[i][4]) > 0)
            	{
						std::cout << mServoValues[i][0] << ',' << mServoValues[i][1] << ',' << mServoValues[i][2] << ',' 
									 << mServoValues[i][3] << ',' << mServoValues[i][4] << std::endl;
            		i++;
            	}
